...
def get_lane_center(cv_image: np.array, detection_height: int, detection_thickness: int, road_gradient: float, lane_width: int) -> int:
	# lane_width 는 '차선 폭'이지 이미지 폭이 아니다. 기본값과 범위 제한에는
	# 실제 이미지 폭을 써야 한다. (기존 코드는 둘을 혼동해서, 차선을 놓치면
	# 목표점이 150 으로 가고 한쪽 차선만 보이면 299 를 넘지 못했다. 차 중심은 320 이다.)
	...
	if (detected_x_coords.shape[0] < 5):
		...
		center_pixel = None
		return image_width/2   # 차선을 못 찾으면 정면 유지
	...
	if (center_pixel == None) & (line_x_axis_pixel == None):
		road_target_point_x = (image_width/2)
	else:
		...
		if (road_target_point_x == None) & (line_x_axis_pixel != None):
			if road_gradient > 0:
				...
				road_target_point_x = line_x_axis_pixel - (lane_width/2)
			if road_target_point_x > (image_width-1):
				road_target_point_x = (image_width-1)
...
# 가장 큰 색 비율이 이 값 미만이면 판정 불가로 본다. 신호등 몸체가 crop 대부분을
# 차지하므로 실제 점등 비율은 생각보다 작다. light_probe.py 로 실측해서 조정할 것.
...
def get_traffic_light_color(cv_image: np.array, bbox, hsv_ranges: dict) -> str:
	...
	# bbox 가 화면 밖으로 나가면 crop 이 비어서 roi.size == 0 -> ZeroDivisionError 로 죽는다.
	...
	# 세 비율이 모두 바닥이면 색을 못 읽은 것이다. 이 가드가 없으면
	# max_ratio == red_ratio 가 먼저 참이 되어 무조건 "Red" 를 반환한다.
	if max_ratio < MIN_COLOR_RATIO:
		return "Unknown"

	# 동점일 때 Red 가 먼저 걸리도록 순서를 유지한다. Green 을 먼저 두면
	# 애매한 프레임에서 차가 출발해버리므로 안전한 쪽으로 기울여 둔다.
